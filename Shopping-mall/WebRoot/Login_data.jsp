    ...
    <div class="contain">
        ...
        <table class="table">
            ...
            <%
  			...
  			ArrayList<Goods_bean> list = dao.findSome(pages,10);
			...
			while(it.hasNext()){
			 ...
			 %>		
                <tr>
                    ...
                    <td><%=bean.getPrice() %></td>
                    <td><%=bean.getNum() %></td>
                    <td><a href="Login.jsp" target="_top">加购</a></td>
                </tr>
             <%} %>
            </tbody>
        </table>
        ...
        <div class="pageNav">
            <ul class="pagination">       
                <li class="page-item"><a class="page-link" href="Goods?pagesNow=<%=pagesNow%>&pagesCount=<%=pagesCount%>&op=jump&handle=pre">上一页</a></li>
                <%
					...
					for(int i = minpages;i <=maxpages ;i++){
						if(i == pagesNow){//当前页和遍历出来的页数相等时，需要通过调用css里面的样式“active"进行高亮
							...
							out.print("<a class='page-link active' href='Goods?sort=All&op=click&pages="+i+"'>"+i+"</a>");
							out.print("</li>");
						}else{//输出每一个分页
						out.print("<li class='page-item'>");
							out.print("<a class='page-link' href='Goods?sort=All&op=click&pages="+i+"'>"+i+"</a>");
						}	out.print("</li>");				
					}
                ...
                <li class="page-item"><a class="page-link" href="Goods?pagesNow=<%=pagesNow%>&pagesCount=<%=pagesCount%>&sort=All&op=jump&handle=next">下一页</a></li>
            </ul>
        </div>
