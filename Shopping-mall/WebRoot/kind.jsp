    ...
    <div class="contain">
        ...
        <table class="table">
            ...
            System.out.println("kind="+kind);
  			...
  			int lastRow = dao.getRowBy_kind(kind);
			System.out.println("哈 "+kind);
  			ArrayList<Goods_bean> list = dao.selectBy_kind(pages,pageSize,kind);	
			...
			while(it.hasNext()){
			 ...
			 %>		
                <tr>
                    ...
                    <td><%=bean.getPrice() %></td>
                    <td><%=bean.getNum() %></td>
                    <td><a href="Add_cart?sales_id=<%=id %>&name=<%=bean.getName() %>&from=kind.jsp&from=kind.jsp&op=add&kind=<%=kind %>">加购</a></td>
                </tr>
             <%} %>
            </tbody>
        </table>
        ...
        <div class="pageNav">
            <ul class="pagination">       
                <li class="page-item"><a class="page-link" href="Sort?pagesNow=<%=pagesNow%>&pagesCount=<%=pagesCount%>&op=jump&handle=pre&sort=kind&kind=<%=kind %>">上一页</a></li>
                <%
					...
					for(int i = minpages;i <=maxpages ;i++){
						if(i == pagesNow){//当前页和遍历出来的页数相等时，需要通过调用css里面的样式“active"进行高亮
							...
							out.print("<a class='page-link active' href='Sort?kind="+kind+"&op=click&sort=kind&pages="+i+"'>"+i+"</a>");
							out.print("</li>");
						}else{//输出每一个分页
						out.print("<li class='page-item'>");
							out.print("<a class='page-link' href='Sort?kind="+kind+"&op=click&sort=kind&pages="+i+"'>"+i+"</a>");
						}	out.print("</li>");				
					}
                ...
                <li class="page-item"><a class="page-link" href="Sort?pagesNow=<%=pagesNow%>&pagesCount=<%=pagesCount%>&op=jump&handle=next&sort=kind&kind=<%=kind %>">下一页</a></li>
            </ul>
        </div>
